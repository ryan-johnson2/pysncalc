...
def gen_new_subnet(base_ip, base_mask, num_hosts):
    ...
    if (new_cidr >= old_cidr and new_cidr <= 32):
        ...
        gen_subnet_data(base_ip, new_mask)
...
def gen_new_subnet_cidr(base_ip_cidr, num_hosts):
    ...
    gen_new_subnet(base_ip, base_mask, num_hosts)
...
def print_help():
    ...
    print("{0} -n <IP> <MASK> <# HOSTS>     Generate a new subnet from the base IP and MASK to accomodate # HOSTS".format(sys.argv[0]))
    print("{0} -nc <IP>/<CIDR> <#HOSTS>     Generate a new subnet from the base IP/CIDR to accomodate # HOSTS".format(sys.argv[0]))
    print("\nExamples:\n")
    print("{0} -i 192.168.1.55 255.255.255.0".format(sys.argv[0]))
    print("{0} -i 192.168.1.55/24".format(sys.argv[0]))
    print("{0} -n 192.168.1.0 255.255.255.0 65".format(sys.argv[0]))
    print("{0} -nc 192.168.1.0/24 65".format(sys.argv[0]))
...
def main():
    if len(sys.argv) < 2:
        ...
        if len(sys.argv) < 5:
            ...
            gen_new_subnet(sys.argv[2], sys.argv[3], int(sys.argv[4]))

    elif sys.argv[1] == '-nc':
        if len(sys.argv) < 4:
            print_help()
        else:
            gen_new_subnet_cidr(sys.argv[2], int(sys.argv[3]))
